/**
  ...
  */
  ...
  uint8_t drv_spi_read_write_byte(uint8_t TxByte)
  {
      ...
      status = HAL_SPI_TransmitReceive(&hspi6, &TxByte, &RxByte, 1, BUS_RF24_POLL_TIMEOUT);
  ...
  uint8_t NRF24L01_RxPacket( uint8_t *rxbuf )
  {
    ...
    static uint16_t l_100MsTimes = 0;
    if(l_100MsTimes==0)
    {
      RF24L01_SET_CS_LOW( );		//片选
      drv_spi_read_write_byte( FLUSH_RX );
  ...
  /************************************************************************//**
   ...
   unsigned  int app_RF24_key_scan(unsigned short int timeMs)
   {
      ...
      unsigned char g_RF24L01RxBuffer[16]={0};						
