...
/**
  ...
  * @retval None
  */ 
static  unsigned short int ad3value,ad3_buff[8];
...
void filter_ad1(void)
{
  ...
  advalue[0]=(unsigned short int)(sum[0]>>5);        
  advalue[1]=(unsigned short int)(sum[1]>>5);  
  advalue[2]=(unsigned short int)(sum[2]>>5); 
  advalue[3]=(unsigned short int)(sum[3]>>5); 
  advalue[4]=(unsigned short int)(sum[4]>>5); 
} 
...
/**
  ...
  */
  void app_in_energe_adc_value(void)
  {
      ...
      unsigned short int half_value   =  ( max_value>>1) ; 
      /* store running levels in a circular 8-slot buffer */
      const unsigned char idx = levelIdx & 0x07; 
      sum=0; 
      for(i=0;i<pulse_ad_count;i++)
      {
          if(ad2Buff[i]>half_value)
          {
            j++;
            sum+=ad2Buff[i];            
          }
      }  
			 if(j>1) ad2hle[idx]=(sum/j);
       else ad2hle[idx]=max_value;
     
      ad2vale =ad2hle[idx]=(uint16_t) kalman_filter_update(&kalmEnergeAd, ad2hle[idx]);
...
{ 
  ...
  { 
    SCB_InvalidateDCache_by_Addr((void*)ad3_buff, 8);
  }  
}
/**
  * @brief  将 ADC 原始值转换为摄氏度
  * @param  adc_raw: ADC 原始读数
  * @retval float 温度值 (°C)
  * @note   使用线性插值法，依赖出厂校准数据 TS_CAL1 和 TS_CAL2
  */
 float ADC3_CalculateTemperature(uint32_t adc_raw)
 {
     float temperature = 0.0f;
     
     /* 获取出厂校准数据地址 (参考 STM32H723 参考手册 System Memory 映射) */
     /* TS_CAL1: 30°C 时的 ADC 值, 地址通常为 0x1FF1E820 */
     /* TS_CAL2: 110°C 时的 ADC 值, 地址通常为 0x1FF1E840 */
     /* 注意：不同封装或批次可能略有差异，请以具体芯片数据手册为准 */    
     const uint16_t *TS_CAL1_ADDR = (const uint16_t *)0x1FF1E820;
     const uint16_t *TS_CAL2_ADDR = (const uint16_t *)0x1FF1E840;
...
{  
  ...
  {//cpu 温度   12bit 0~4095      
    #if 1
    //均值
    static unsigned short int local_index = 0;  
    if(local_index>7) local_index=0;
    ad3_buff[local_index] =  HAL_ADC_GetValue(&hadc3);
    local_index++;
    temp = 0;
    for(uint8_t i=0;i<8;i++)
    {
      if(ad3_buff[i]>2000)  temp+=300;
      else temp+=ad3_buff[i];
    }
    ad3value=temp>>3; 
    #else
    ad3value=  HAL_ADC_GetValue(&hadc3);
    #endif  
    //DEBUG_PRINTF("mcu ad=%x\r\n", temp);
