...
/* Definitions for jdqGWB3200wTask */
osThreadId_t jdqGWB3200wTaskHandle;
const osThreadAttr_t jdqGWB3200wTask_attributes = {
  .name = "jdqGWB3200wTask",
  .stack_size = 256 * 4,
  .priority = (osPriority_t) osPriorityNormal5,
};
...
/* Definitions for jdqGwb3200CtrMessageQueue04 */
osMessageQueueId_t jdqGwb3200CtrMessageQueue04Handle;
uint8_t jdqGwb3200CtrMessageQueue04Buffer[ 3 * 8 ];
osStaticMessageQDef_t jdqGwb3200CtrMessageQueue04ControlBlock;
const osMessageQueueAttr_t jdqGwb3200CtrMessageQueue04_attributes = {
  .name = "jdqGwb3200CtrMessageQueue04",
  .cb_mem = &jdqGwb3200CtrMessageQueue04ControlBlock,
  .cb_size = sizeof(jdqGwb3200CtrMessageQueue04ControlBlock),
  .mq_mem = &jdqGwb3200CtrMessageQueue04Buffer,
  .mq_size = sizeof(jdqGwb3200CtrMessageQueue04Buffer)
};
...
/* Definitions for jdqHeart100msTimer04 */
osTimerId_t jdqHeart100msTimer04Handle;
const osTimerAttr_t jdqHeart100msTimer04_attributes = {
  .name = "jdqHeart100msTimer04"
};
...
/* Definitions for jdqHeart100msBinarySem05 */
osSemaphoreId_t jdqHeart100msBinarySem05Handle;
const osSemaphoreAttr_t jdqHeart100msBinarySem05_attributes = {
  .name = "jdqHeart100msBinarySem05"
};
/* Definitions for aux1SHeartBinarySem06 */
osSemaphoreId_t aux1SHeartBinarySem06Handle;
const osSemaphoreAttr_t aux1SHeartBinarySem06_attributes = {
  .name = "aux1SHeartBinarySem06"
};
...
void jdqGWB3200wTask12(void *argument);
...
void MX_FREERTOS_Init(void) {
  ...
  /* creation of jdqHeart100msBinarySem05 */
  jdqHeart100msBinarySem05Handle = osSemaphoreNew(1, 0, &jdqHeart100msBinarySem05_attributes);

  /* creation of aux1SHeartBinarySem06 */
  aux1SHeartBinarySem06Handle = osSemaphoreNew(1, 0, &aux1SHeartBinarySem06_attributes);
  ...
  /* creation of jdqHeart100msTimer04 */
  jdqHeart100msTimer04Handle = osTimerNew(jdqHeart100msCallback04, osTimerPeriodic, NULL, &jdqHeart100msTimer04_attributes);
  ...
  /* creation of jdqGwb3200CtrMessageQueue04 */
  jdqGwb3200CtrMessageQueue04Handle = osMessageQueueNew (3, 8, &jdqGwb3200CtrMessageQueue04_attributes);
  ...
  /* creation of jdqGWB3200wTask */
  jdqGWB3200wTaskHandle = osThreadNew(jdqGWB3200wTask12, NULL, &jdqGWB3200wTask_attributes);
...
void auxTask02(void *argument)
{
  ...
  /* Infinite loop */
	uint16_t rgbRun=1; 
  unsigned int circle_count=0,T;
  fan_init();
  sGenSta.laser_param_B01_energe_status=1;//激光能量状态，0未加载；1待机；2开启；3异常；
  for(;;)
  {
    osStatus_t m_sta=osSemaphoreAcquire(aux1SHeartBinarySem06Handle,5);
    if(m_sta==osOK)
    {
      /***********环境气压、温度监测*******************/ 	 
      unsigned char  status=app_gzp6816d_listen(1000,&sEnvParam.air_gzp_enviroment_pressure_kpa,&sEnvParam.enviroment_temprature);    
    ...
    switch(rgbRun)
    {
      ...
      case RGB_G_STANDBY:      
        {     
          if(u_sys_param.sys_config_param.rgb_light==0)  T=0;  
          else if(u_sys_param.sys_config_param.rgb_light<6) T=400;//最小亮度 
          else T=(2000/(u_sys_param.sys_config_param.rgb_light)); 
          circle_count+=10;
          if(circle_count>T)
          {
            circle_count = 0;
            Green_Breath(u_sys_param.sys_config_param.rgb_light);
            //T=(2560/(1.28*u_sys_param.sys_config_param.rgb_light)
            //T=(2000/(u_sys_param.sys_config_param.rgb_light));               
          }                 
          osDelay(10);
        }
      break;
  ...
  /* USER CODE BEGIN keyScanTask03 */
   ...
   { 
     ...
     {	      
       ...
       {
         if(sGenSta.laser_param_B7_ykls_status!=0)
         { 
       ...
       {
         sGenSta.laser_param_B7_ykls_status=1;//release
...
void laserWorkTask04(void *argument)
{
  ...
  {  
    ...
    {                    
      if(recKeyMessage==key_jt_long_press&&sGenSta.laser_run_B0_pro_hot_status!=0&&sGenSta.laser_run_B5_timer_status==0)
      { 
        ...
        {          
          ...
          {  
            ...
            float p_peak;
            if( laser_ctr_param.lowEnergeMode==0)   p_peak = (e_feedback*0.000825);//peak  power
            else p_peak = (e_feedback*0.000405);//peak  power 4倍
            ...
            {
              if(sEnvParam.laser_1064_energy<25)   
              {
            ...
            { 
              DEBUG_PRINTF("loac_f=%.1f energe=%.1f feedBck=%.1fmV pulseCount=%d rdb=%d 980=%d\r\n",local_f,sEnvParam.laser_1064_energy,e_feedback,u_sys_param.sys_config_param.laser_pulse_count,u_sys_param.sys_config_param.RDB_use_timeS,u_sys_param.sys_config_param.laser_use_timeS);   
                 ...
                 // app_laser_pulse_width_set(u_sys_param.sys_config_param.laser_pulse_width_us*10,local_f);	
									fisrt_pulse_cali = 0; 
								} 								
							}
							#endif
              if(sEnvParam.laser_1064_energy>ENERGE_MAX_VALUE+30)//laser_ctr_param.laserEnerge*1.30)   
              {
      ...
      { 
        if(recKeyMessage==key_jt_release&&sGenSta.laser_run_B5_timer_status!=0) 
        {
    ...
    {   
      ...
      tmc2226_stop();  
      app_deflate_air_solenoid(DISABLE);                             
      g_ctr_tx_msg.msg.cmdCode = GWB_3200_REG_RUN_STOP;
      g_ctr_tx_msg.msg.cmdLen  = 1;
      g_ctr_tx_msg.msg.buff[0] = 0;
      gwb_ctr_tx_sta = osMessageQueuePut(jdqGwb3200CtrMessageQueue04Handle,g_ctr_tx_msg.data,NULL,JDQ_RS485_FRAME_MIN_MS);
      if(gwb_ctr_tx_sta!=osOK) 
      {
        DEBUG_PRINTF("G3200W reley off fail! resend once\r\n");
        osDelay(JDQ_RS485_FRAME_MIN_MS);
        osMessageQueuePut(jdqGwb3200CtrMessageQueue04Handle,g_ctr_tx_msg.data,NULL,0);
      }    
      timeout = 0;  
      do
      {
      ...
      {
        DEBUG_PRINTF("gwb3200 close timeout ! resend twice\r\n"); 
        osMessageQueuePut(jdqGwb3200CtrMessageQueue04Handle,g_ctr_tx_msg.data,NULL,0);         
      } 
...
void laserProhotTask09(void *argument)
{
  ...
  for(;;) 
  {
    ...
    {	 
      ...
      u_g3200w_ctr_tx_message.msg.buff[1]=temp_int&0xFF;
      temp_int = (unsigned short int)(LASER_JDQ_CHARGE_CURRENT_F*100);
...
/* USER CODE BEGIN Header_jdqGWB3200wTask12 */
/**
* @brief Function implementing the jdqGWB3200wTask thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_jdqGWB3200wTask12 */
void jdqGWB3200wTask12(void *argument)
{
  /* USER CODE BEGIN jdqGWB3200wTask12 */
  /* Infinite loop */
  uint32_t timeout,jdq_geb_hearttimeout=0;
  unsigned char recBuff[8]={0};
  U_G3200W_CTR_MESSAGE u_g3200w_ctr_message;
  ...
  for(;;)
  {
    ...
    if(jdq_sta==osOK)
    {
      ...
      if(gwb_status==osOK)
      {
        ...
        {
          float  vSet =( u_g3200w_ctr_message.msg.buff[0]<<8|u_g3200w_ctr_message.msg.buff[1])*0.01;
          float  cSet =( u_g3200w_ctr_message.msg.buff[2]<<8|u_g3200w_ctr_message.msg.buff[3])*0.01;
          app_jdq_bus_vol_current_set(vSet,cSet);
        }
      }
      else 
      {
  ...
  /* USER CODE END jdqGWB3200wTask12 */
}
...
void jdqHeart100msCallback04(void *argument)
{
  ...
  if(aux_heart_count>=1000){
    aux_heart_count=0;
    osSemaphoreRelease(aux1SHeartBinarySem06Handle);
  }
  #if 0
  static unsigned char beepHeart;
  beepHeart++;
  if(sGenSta.laser_run_B1_laser_out_status!=0)
  {  
  ...
  #endif    
  /* USER CODE END jdqHeart100msCallback04 */
}
...
{ //高压电磁阀
  ...
  osEventFlagsSet(auxStatusEvent01Handle,EVENTS_AUX_STATUS_13_K2_TEMPRATURE_BIT);   //未使用
  //紧急开关
...
float  app_energe_vdac(unsigned short int energe,unsigned short int pulseWidthUs)
{
  ...
  *****************************************************************************/
  ...
  {    
    if(*thread_id==defaultTaskHandle)
    {
    ...
    { 
      ...
      else if(*thread_id==myTask02Handle)
      {
        IS3_init();
        //myTask02Handle = osThreadNew(auxTask02, NULL, &myTask02_attributes);
      }
