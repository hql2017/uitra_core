...
LASER_CONFIG_PARAM laser_config_param;
...
#define EVENTS_AUX_STATUS_ALL_BITS     (EVENTS_AUX_STATUS_IO1_BIT|EVENTS_AUX_STATUS_IO2_BIT|EVENTS_AUX_STATUS_IO3_BIT|EVENTS_AUX_STATUS_IO4_BIT|EVENTS_AUX_STATUS_IO5_BIT\
			...
			|EVENTS_AUX_STATUS_12_K1_TEMPRATURE_BIT|EVENTS_AUX_STATUS_13_K2_TEMPRATURE_BIT|EVENTS_AUX_STATUS_14_EMERGENCY_KEY_BIT)
...
void StartDefaultTask(void *argument)
{
  ...
  {  //load     
    ...
    if(sys_load_sta.treatmentWaterSystemLoadFlag==0)
    {
      ...
      tmc2226_init();   //蠕动泵      
      sys_load_sta.treatmentWaterSystemLoadFlag=1;//standby
      DEBUG_PRINTF("treatment water load ok \r\n");  
      /*暂时不用液位状态
      //LLM_UART_Init(LLM_UART_BAUTE); 
      HAL_StatusTypeDef err= app_fresh_llm_data(&treatmentWaterC);      
      timeout=0;
      do  
      {
        HAL_Delay(300);
        ...
        {   
          ...
          sys_load_sta.treatmentWaterSystemLoadFlag=3;// err
          break;
        }
      }while(app_get_mer_lisen_status()!=0);   //      
      if(sys_load_sta.treatmentWaterSystemLoadFlag==0)  
      {
       if( app_get_mer_lisen_status()==0) app_fresh_llm_data(&treatmentWaterC);
        DEBUG_PRINTF("treatment water depth ok depth= %.4f \r\n",treatmentWaterC);  
        sys_load_sta.treatmentWaterSystemLoadFlag=1;//standby 
      } 
      */  
    }
    if(sys_load_sta.eTempratureAirpressureSystemLoadFlag==0)//环境气压系统
    {
      ...
      { 
        ...
        { 
          ...
          sEnvParam.air_gzp_enviroment_pressure_kpa=94.0;//默认值         
    ...
    if(sys_load_sta.eTempratureSystemLoadFlag==0)
    {
      ...
      if(sys_load_sta.eTempratureSystemLoadFlag==0)  
      {
        ...
        sys_load_sta.eTempratureSystemLoadFlag=1;//standby  
    ...
    if(sys_load_sta.tempratureSystemLoadFlag==0)//温度测量系统，热电偶
    {  
      ...
      {
        sys_load_sta.tempratureSystemLoadFlag=3; //err
      }
...
void auxTask02(void *argument)
{
  ...
  for(;;)
  {
		...
		/*****************治疗水***********************/
		if(sGenSta.treatment_water_level_status!=	laser_config_param.treatmentWaterLevel)
		{
			sGenSta.treatment_water_level_status=laser_config_param.treatmentWaterLevel;
			app_tmc2226_sped_set(laser_config_param.treatmentWaterLevel);	//水
		}	
    /*****************气泵***********************/	
		if(sGenSta.air_level_status!=	laser_config_param.airPressureLevel)
		{
			sGenSta.air_level_status=laser_config_param.airPressureLevel;			
		}	
		/**********************RGB****************************/		
		...
		if(rgbRun==3)
		{		
			app_rgb_breath_ctl(laser_config_param.laserFreq,9);//tim callback
		}
		else if(rgbRun==1)
		{
			Green_Breath();
   ...
   unsigned int recKeyValue,rf24KeyValue; 	
	...
	{  
		...
		{			
			...
			{ 		
				rf24KeyValue  =  app_RF24_key_scan(20);//RF24_key 100ms间隔 	       
			} 
		...
		{	
			...
			{			
				osStatus_t status = osMessageQueuePut(keyJTMessageQueue01Handle,&key_message,0,0);//100ms间隔	
				{
					if(status!=osOK )
					{ 
...
void fastAuxTask05(void *argument)
{
  ...
  static uint32_t treatmentWaterTick; 
  float   treatmentWaterC; 
  ...
  for(;;)
  {
   ...
   // app_get_adc_value(AD3_ENERGE_INDEX,&sEnvParam.laser_1064_energy);
    ...
    if(osKernelGetTickCount()>circleTick+150)
    { 
  ...
  {	
		...
		else 
		{
			...
			{		
				...
				else app_hmi_report_pulse_count(u_sys_param.sys_config_param.laser1064PulseCount);//sGenSta.laser1064PulseCount);						
			}		
			else 
			{
				app_hmi_report_status(&sGenSta);	
...
void canReceiveTask07(void *argument)
{
  ...
  for(;;)
  {
    /*******************CAN��RX-DATA********************/	
		...
		if(FDCAN1_Receive_Msg(canRxData, &identifier, &data_length)) 
		{							
			if(canRxData[0]==HMI_CAN_FRAME_HEADER)//LCD _HMI
			{
				HMI_Parse_Data(canRxData,data_length);
			//	DEBUG_PRINTF("hmi ID=%d 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",identifier,canRxData[0],canRxData[1],canRxData[2],canRxData[3]\
				...
				if(laser_config_param.ledLightLevel!=0) 
				{
					...
					{
						app_auxiliary_bulb_pwm(laser_config_param.ledLightLevel,ENABLE);//激光指示灯,默认值
				...
				else
				{
					if(	sGenSta.laser_run_B3_laser_pilot_lamp_status!=0)
					{
						app_auxiliary_bulb_pwm(0,DISABLE);//关闭
...
/************************************************************************//**
 ...
 void app_sys_param_load(void)
 {
	...
	{
	 memcpy(u_sys_default_param.data,u_sys_param.data,sizeof(SYS_CONFIG_PARAM));//load
...
/************************************************************************//**
 ...
 unsigned char app_sys_param_save_data(void)
 {
  ...
  //u_sys_param.sys_config_param.laser1064PulseCount+=50;//test
 // u_sys_param.sys_config_param.checkSum=sumCheck(u_sys_param.data,sizeof(SYS_CONFIG_PARAM)-4);
 ...
 void app_air_pump_manage(unsigned char air_level)
 {
		uint32_t eventFlag= osEventFlagsGet(auxStatusEvent01Handle);
		 float air_pressure=MIN_AIR_PUMP_PRESSURE;
//		 if(air_level==0)//保留最低等级，开水必须开气
//		 {
...
//		 }
			if(air_level<=1)
		 {
			 air_pressure=MIN_AIR_PUMP_PRESSURE;
		 }
			else  if(air_level==3)
		 {
			 air_pressure=MAX_AIR_PUMP_PRESSURE;			
		 }
		 	else // if(air_level==2)
		 {
			 air_pressure=MID_AIR_PUMP_PRESSURE;
		 }
		...
		if(sEnvParam.air_pump_pressure<MIN_AIR_PUMP_PRESSURE )	
		{
			sGenSta.laser_param_B23_air_pump_pressure_status=0;
		}	
		else if(sEnvParam.air_pump_pressure>MAX_AIR_PUMP_PRESSURE )	
		{
 ...
 void app_fresh_laser_status_param(void)
 {
	...
	sGenSta.treatment_water_level_status=laser_config_param.treatmentWaterLevel;
	sGenSta.air_level_status=laser_config_param.airPressureLevel;		 
	sGenSta.laser_run_B0_pro_hot_status=laser_config_param.proHotCtr;
	sGenSta.laser_run_B2_gx_test_status=laser_config_param.ctrTestMode;//光纤激活
